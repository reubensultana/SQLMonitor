...
IF OBJECT_ID('[Staging].[IndexUsageStats]') IS NOT NULL
BEGIN
    DROP TABLE [Staging].[IndexUsageStats];
END
GO

CREATE TABLE [Staging].[IndexUsageStats](
...
BEGIN
    ...
    MERGE INTO [Monitor].[IndexUsageStats] WITH (HOLDLOCK) AS TargetTable
    USING (
        SELECT 
            [ServerName] 
            ,[DatabaseName]
        ...
        FROM inserted -- [Staging].[IndexUsageStats]
        ) AS SourceTable (
            [ServerName] 
            ,[DatabaseName]
            ,[ObjectName] 
