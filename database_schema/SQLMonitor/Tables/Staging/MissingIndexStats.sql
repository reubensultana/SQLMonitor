...
IF OBJECT_ID('[Staging].[MissingIndexStats]') IS NOT NULL
BEGIN
    DROP TABLE [Staging].[MissingIndexStats];
END
GO

CREATE TABLE [Staging].[MissingIndexStats](
...
BEGIN
    ...
    MERGE INTO [Monitor].[MissingIndexStats] WITH (HOLDLOCK) AS TargetTable
    USING (
        SELECT 
            [ServerName] 
            ,[DatabaseName]
        ...
        FROM inserted -- [Staging].[MissingIndexStats]
        ) AS SourceTable (
            [ServerName] 
            ,[DatabaseName]
	        ,[ObjectName]
