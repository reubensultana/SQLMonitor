...
CREATE TABLE #SampleDatabases ( [DatabaseName] nvarchar(128) );
INSERT INTO #SampleDatabases( [DatabaseName] )
SELECT [name] FROM sys.databases WHERE [name] LIKE 'AdventureWorks%' AND [name] IN ('pubs', 'Northwind');
...
FROM #databaseinfo 
WHERE ([database_name] IN (SELECT [DatabaseName] FROM #SampleDatabases));

-- remove last comma
IF LEN(@msbresult) >0 
    SET @msbresult = LEFT(@msbresult, LEN(@msbresult)-1)

IF EXISTS (SELECT 1 FROM #databaseinfo WHERE ([database_name] IN (SELECT [DatabaseName] FROM #SampleDatabases)))
	INSERT INTO #MSBChecks (msb_id, msb_InstanceName, msb_name, msb_check, msb_result, msb_compliant)
...
--3.10 - Database Mail
CREATE TABLE #databasemail ( [Status] NVARCHAR (128) );

IF (@EngineEdition IN (1,2))
BEGIN
    BEGIN TRY
...
BEGIN
    BEGIN TRY
        INSERT INTO #tracemessage (
            [auto_start],[msx_server_name],[sqlagent_type],[startup_account],[sqlserver_restart],[jobhistory_max_rows],[jobhistory_max_rows_per_job],
            [errorlog_file],[errorlogging_level],[error_recipient],[monitor_autostart],[local_host_server],[job_shutdown_timeout],[cmdexec_account],
            [regular_connections],[host_login_name],[host_login_password],[login_timeout],[idle_cpu_percent],[idle_cpu_duration],[oem_errorlog],
            [sysadmin_only],[email_profile],[email_save_in_sent_folder],[cpu_poller_enabled],[alert_replace_runtime_tokens] )
...
INSERT INTO #eventforwarding (
    [AlertFailSafeOperator], [AlertNotificationMethod], [AlertForwardingServer], [AlertForwardingSeverity], [AlertPagerToTemplate], 
    [AlertPagerCCTemplate], [AlertPagerSubjectTemplate], [AlertPagerSendSubjectOnly], [AlertForwardAlways])
...
SET @msbresult = CONVERT(nvarchar(128), (SELECT COUNT([is_policy_checked]) FROM sys.sql_logins WHERE [is_policy_checked] = 0)) 
IF @msbresult = 0
	INSERT INTO #MSBChecks (msb_id, msb_InstanceName, msb_name, msb_check, msb_result, msb_compliant)
...
SET @DatabaseNames = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
	...
	AND [name] NOT IN (SELECT [DatabaseName] FROM #SampleDatabases)
...
BEGIN
    SELECT msb_InstanceName AS [ServerName]
		,msb_id AS [MSBID]
		,msb_name AS [MSBName]
		,msb_check AS [MSBCheck]
		,msb_result AS [MSBResult]
        ,CASE msb_compliant 
            WHEN -1 THEN 'Manual'
            WHEN 0 THEN 'Failed'
            WHEN 1 THEN 'Pass'
